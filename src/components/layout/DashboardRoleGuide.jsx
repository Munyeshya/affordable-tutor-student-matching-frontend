...
export function DashboardRoleGuide({ open, role, onClose }) {
  ...
  const guide = dashboardRoleGuides[String(role || '').toUpperCase()] || dashboardRoleGuides.STUDENT
        ...
        <div className="dashboard-guide-body">
          <section aria-labelledby="guide-actions-title">
            ...
            <div className="dashboard-guide-actions">
              {guide.actions.map(({ title, description, to, linkLabel }, index) => (
                <article key={title}>
                  <span>{String(index + 1).padStart(2, '0')}</span>
                  <div>
                    <h4>{title}</h4>
                    <p>{description}</p>
                    <Link to={to} onClick={onClose}>{linkLabel}</Link>
                  </div>
                </article>
              ))}
            </div>
          ...
          <aside aria-labelledby="guide-limits-title">
            ...
            <p><strong>Helpful tip</strong><br />{guide.tip}</p>
