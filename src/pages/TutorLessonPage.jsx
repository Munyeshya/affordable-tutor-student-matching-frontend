      ...
      <div className="lesson-editor-layout">
        ...
        <section className="lesson-editor-panel">
          ...
          <form onSubmit={(event) => { event.preventDefault(); saveMutation.mutate() }}>
            ...
            <label><span>External video URL</span><input disabled={!editable} type="url" value={activeForm.video_url} onChange={(event) => setForm((current) => ({ ...(current || initialForm), video_url: event.target.value }))} placeholder="https://..." /><small>YouTube, Vimeo, Loom, and direct video links play inside the learner workspace.</small></label>
